# =============================================
# DELETE ALL USERS FROM DATABASE - Final Version
# =============================================
...
try {
    ...
    # Show all users before deletion (using correct column names)
    Write-Host "`nUsers that will be deleted:" -ForegroundColor Yellow
    $selectQuery = "SELECT ID, FirstName, LastName, Email, Phone, NationalID, DateOfBirth, CreatedDate FROM Patients ORDER BY CreatedDate DESC"
    $selectCommand = New-Object System.Data.SqlClient.SqlCommand($selectQuery, $connection)
    $reader = $selectCommand.ExecuteReader()
    
    while ($reader.Read()) {
        Write-Host "ID: $($reader['ID']) | Name: $($reader['FirstName']) $($reader['LastName']) | Email: $($reader['Email'])" -ForegroundColor White
    }
    $reader.Close()
